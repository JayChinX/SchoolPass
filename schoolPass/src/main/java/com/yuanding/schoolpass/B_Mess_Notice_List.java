...
import com.yuanding.schoolpass.service.Api.InterNoticeDelete;
import com.yuanding.schoolpass.service.Api.InterNoticeList;
import com.yuanding.schoolpass.service.Api.InterUpdateList;
 ...
 */
public class B_Mess_Notice_List extends A_0_CpkBaseTitle_Navi {
...
//				R.drawable.ic_default_empty_bg);
	...
	private void readCache(String level) {
		...
		jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_notice_list+ A_0_App.USER_UNIQID + level);
	...
	private List<Cpk_Notice_List> getList(JSONObject jsonObject) {
			...
			if (state == 1) {
				mlistContact = JSON.parseArray(jsonObject.optJSONArray("mlist")+ "", Cpk_Notice_List.class);
				mCurrentWaitSureNoticeNumber = jsonObject.optInt("unreceipt_num");
			}
			return mlistContact;
	...
	private void getNoticeListData(final String level, int page_no,final boolean pullRefresh,final boolean onlyUpdateCache) {
		...
		A_0_App.getApi().getNoticeList(B_Mess_Notice_List.this,
				A_0_App.USER_TOKEN, level, String.valueOf(page_no),
				new InterNoticeList() {
					@Override
					public void onSuccess(List<Cpk_Notice_List> mList,int unreceipt_num) {
						if (isFinishing())
							return;
						havaSuccessLoadData = true;
						if(!onlyUpdateCache){//更新缓存用
    						...
    						if (mList != null && mList.size() > 0) {
    							...
    							showLoadResult(false, true, false, false);
    							have_read_page = 2;
    							mCurrentWaitSureNoticeNumber = unreceipt_num;
    							clearBusinessList(false);
    							mCourseList = mList;
    							adapter.notifyDataSetChanged();
    							
    							if (pullRefresh)
    								PubMehods.showToastStr(B_Mess_Notice_List.this,"刷新成功");
    						} else {
    							showTitleBt(ZUI_RIGHT_TEXT, false);
								judge_read(mList,firsetEnterLoader);
    							showLoadResult(false, false, false, true);
                    ...
                    public void onFailure(String msg) {
                        ...
                        showTitleBt(ZUI_RIGHT_TEXT, false);
                        //new add
                        PubMehods.showToastStr(B_Mess_Notice_List.this, msg);
                        demo_swiperefreshlayout.setRefreshing(false);  
                        if(null!=mPullDownView){
                            mPullDownView.onRefreshComplete();
                            mPullDownView.setMode(Mode.PULL_UP_TO_REFRESH);
	...
	private void batchUpdate(String level) {
        if (isFinishing())
            return;
        A_0_App.getInstance().showProgreDialog(B_Mess_Notice_List.this, "", true);
		...
		A_0_App.getApi().NoticeDelete(A_0_App.USER_TOKEN, messgesid,
				new InterNoticeDelete() {
					...
					public void onSuccess() {
						...
						PubMehods.showToastStr(B_Mess_Notice_List.this, "删除成功");
					}
				},new Inter_Call_Back() {
...
//						A_0_App.getInstance().CancelProgreDialog(B_Mess_Notice_List.this);
						...
						if (mList != null && mList.size() > 0) {
							...
							PubMehods.showToastStr(B_Mess_Notice_List.this,"没有更多了");
						}
						if(null!=mPullDownView){
		                	mPullDownView.onRefreshComplete();
		                }
		...
		public View getView(int posi, View converView, ViewGroup arg2) {
			...
			currentType = getItemViewType(posi);
				...
				if(currentType==TYPE_HAVEBTN){
					...
					if(null==converView){
						haveWaitSureBtnView = LayoutInflater.from(B_Mess_Notice_List.this).inflate(R.layout.item_notificity_list_waitsurebtn, null);
					...
					//添加数据
					if (null != holder2) {
						if (mCurrentWaitSureNoticeNumber>0) {
							holder2.btn_waitsurenotice.setVisibility(View.VISIBLE);
//							holder2.btn_waitsurenotice.setText(mCurrentWaitSureNoticeNumber+" 条待确认信息");
							holder2.btn_waitsurenotice.setText(PubMehods.matcherSearchTitle(B_Mess_Notice_List.this.getResources().getColor(R.color.main_color), mCurrentWaitSureNoticeNumber+" 条待确认信息", new String[]{mCurrentWaitSureNoticeNumber+""}));
							holder2.btn_waitsurenotice .setOnClickListener(new OnClickListener() {

										@Override
										public void onClick(View arg0) {
											Intent intent = new Intent(B_Mess_Notice_List.this,B_Mess_Notice_List_WaitSure.class);
				    ...
				    if(null==converView){
				    	defaultNoticeMsgView = LayoutInflater.from(B_Mess_Notice_List.this).inflate(R.layout.item_notificity_list_havesurereceiverbtn, null);
				    ...
				    //添加数据
				    final int mPosi = posi-1; //第一个是按钮 减1按照原来的方式取数据
					...
					if (LONGDELETE == 1) {
						...
						int left=DensityUtils.dip2px(B_Mess_Notice_List.this, 20);
			            int top=DensityUtils.dip2px(B_Mess_Notice_List.this, 20);
			            int right=DensityUtils.dip2px(B_Mess_Notice_List.this, 40);
			            int bottom=DensityUtils.dip2px(B_Mess_Notice_List.this, 1);
						layoutParams.setMargins(-left, top, right, bottom);// 4个参数按顺序分别是左上右下
						holder.ll_notice.setLayoutParams(layoutParams);
						int paddingLeft=DensityUtils.dip2px(B_Mess_Notice_List.this, 40);
						int padding=DensityUtils.dip2px(B_Mess_Notice_List.this, 17);
						...
						int left=DensityUtils.dip2px(B_Mess_Notice_List.this, 20);
		                int top=DensityUtils.dip2px(B_Mess_Notice_List.this, 20);
		                int right=DensityUtils.dip2px(B_Mess_Notice_List.this, 0);
		                int bottom=DensityUtils.dip2px(B_Mess_Notice_List.this, 1);
						layoutParams.setMargins(left, top, right, bottom);// 4个参数按顺序分别是左上右下
						int padding=DensityUtils.dip2px(B_Mess_Notice_List.this, 17);
...
//								holder.iv_notice_deatil_img, options);
					...
					if (mCourseList.get(mPosi).getMl_status().equals("1")) {
						...
						if (mCourseList.get(mPosi).getType().equals("4")) { //短信 
							...
							switch (mCourseList.get(mPosi).getIs_appendix()){
								...
								case 1:
									holder.tv_notice_detail_title.setText("" + mCourseList.get(mPosi).getTitle());
									Drawable drawable = ContextCompat.getDrawable(B_Mess_Notice_List.this, R.drawable.file_zhizhen);

									drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());

									holder.tv_notice_detail_title.setCompoundDrawables(null, null, drawable, null);

									holder.tv_notice_detail_title.setCompoundDrawablePadding(25);
									break;
							}
					...
					holder.ll_notice.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							if (mCourseList.get(mPosi).getIs_default().equals("0")) {
								if (mCourseList.get(mPosi).getType().equals("4")) {// 短信文本消息
									Intent intent = new Intent(B_Mess_Notice_List.this,B_Mess_Notice_Detail_MessText.class);
									...
									Intent intent = new Intent(B_Mess_Notice_List.this,B_Mess_Notice_Detail.class);
									intent.putExtra("acy_type", 2);// 正常列表进入
									intent.putExtra("message_id", mCourseList.get(mPosi).getMessage_id());
									intent.putExtra("mess_content",mCourseList.get(mPosi).getDesc());
									startActivityForResult(intent, 1);
								...
								Intent intent = new Intent(B_Mess_Notice_List.this,B_Mess_Notice_Detail_Sys.class);
								intent.putExtra("acy_type", 2);
								intent.putExtra("message_id", mCourseList.get(mPosi).getMessage_id());
								intent.putExtra("log_id", mCourseList.get(mPosi).getLog_id());
								startActivityForResult(intent, 1);
							}
	...
	private void sendSureReceReq(final int posi,String msgid){
		...
		A_0_App.getApi().sendNoticeSureReceIpt(B_Mess_Notice_List.this,
...
//					getNoticeListData(level, 1, false, true);//更新第一页缓存数据
				}
			}else if (requestCode == 2){
				boolean isNeedRefreshData = data.getBooleanExtra("isNeedRefreshData", false);
