...
import com.yuanding.schoolpass.B_Mess_Notice_List.MyAsyncTask;
...
import com.yuanding.schoolpass.service.Api.InterReleaseHistoryTaskListMain;
import com.yuanding.schoolpass.service.Api.InterReleaseTaskListMain;
 ...
 * 帮帮 我发的任务
 */
public class B_Account_Befriend_Center_Main_2_Task_Send extends Fragment{
	...
	private List<Befriend_Release_Task_Bean> mMainToadayList = new ArrayList<Befriend_Release_Task_Bean>();
	private List<Befriend_ReleaseHistroy_Task_Bean> mMainEarlyList = new ArrayList<Befriend_ReleaseHistroy_Task_Bean>();
    ...
    public B_Account_Befriend_Center_Main_2_Task_Send() {
		super();
	}
    ...
    private void readCache( ) {
        ...
        jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_query_release_task+ A_0_App.USER_UNIQID);
        jsonObject_history = maACache.getAsJSONObject(AppStrStatic.cache_key_query_history_release_task+ A_0_App.USER_UNIQID);
    ...
    private void showInfo(JSONObject jsonObject,JSONObject json_history) {
            ...
            List<Befriend_Release_Task_Bean> toadayListTemp = null;
            List<Befriend_ReleaseHistroy_Task_Bean> earlyListTemp = null;
            
            if (state == 1) {
                toadayListTemp = new ArrayList<Befriend_Release_Task_Bean>();
                JSONArray jsonArrayItem_early = jsonObject.optJSONArray("list");
                if (jsonArrayItem_early != null && jsonArrayItem_early.length() > 0) {
                	toadayListTemp = JSON.parseArray(jsonArrayItem_early + "",Befriend_Release_Task_Bean.class);
                }
            }
            if (state_history == 1) {
                earlyListTemp = new ArrayList<Befriend_ReleaseHistroy_Task_Bean>();
                JSONArray jsonArrayItem_early = json_history.optJSONArray("list");

                if (jsonArrayItem_early != null && jsonArrayItem_early.length() > 0) {
                    earlyListTemp = JSON.parseArray(jsonArrayItem_early + "",Befriend_ReleaseHistroy_Task_Bean.class);
                }
            }
    ...
    private void goActionData(List<Befriend_Release_Task_Bean> mToadayList, List<Befriend_ReleaseHistroy_Task_Bean> mEarlyList) {
        ...
        if (mMainEarlyList.size()==0) {
        	Befriend_ReleaseHistroy_Task_Bean acquire_Task_Bean=new Befriend_ReleaseHistroy_Task_Bean();
        	acquire_Task_Bean.setId("yd");
        	acquire_Task_Bean.setCreateTime("1111111111");
        	acquire_Task_Bean.setCreateUsername("");
        	acquire_Task_Bean.setTaskStatus("1");
    ...
    //获取待处理数据
   	private void getReleaseTaskList() {
   	    A_0_App.getApi().getReleaseTaskListMain(getActivity(), A_0_App.USER_TOKEN,new InterReleaseTaskListMain() {
               @Override
               public void onSuccess(List<Befriend_Release_Task_Bean> release_Task_Beans) {
               	if(getActivity()==null||getActivity().isFinishing())
                       return;
	...
	private void getReleaseHistoryTaskList(final int page_no) {
	    A_0_App.getApi().getReleaseHistoryTaskListMain(getActivity(), A_0_App.USER_TOKEN, String.valueOf(page_no), new InterReleaseHistoryTaskListMain() {
            @Override
            public void onSuccess(List<Befriend_ReleaseHistroy_Task_Bean> release_Task_Beans) {
            	if(getActivity()==null||getActivity().isFinishing())
                    return;
    ...
    public class MyIndexAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertview, ViewGroup arg2) {
            ...
            if (mMainToadayList.get(position).getHeoTaskWorkCreateUsername()!=null&&mMainToadayList.get(position).getHeoTaskWorkCreateUsername().length()>0) {
            	   holder.tv_name.setText("帮忙人:"+mMainToadayList.get(position).getHeoTaskWorkCreateUsername());
			}else{
				 holder.tv_name.setText("帮忙人:"+"暂无领取人");
			}
         
            holder.tv_time.setText(PubMehods.getFormatDate(Long.valueOf(mMainToadayList.get(position).getCreateTime()), "MM/dd HH:mm"));
            switch (mMainToadayList.get(position).getTaskStatus()) {
			case "1":
				holder.tv_wait_money.setText("待领取");
				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.attence_later));
				break;
			case "2":
				holder.tv_wait_money.setText("服务中");
				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.attence_later));
				break;
			case "3":
				holder.tv_wait_money.setText("待确认付款");
				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.attence_later));
				break;
			case "4":
				holder.tv_wait_money.setText("已付款");
				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.title_no_focus_login));
				break;
			case "5":
				holder.tv_wait_money.setText("已失效");
				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.title_no_focus_login));
				break;
			case "6":
				holder.tv_wait_money.setText("任务违规");
				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.title_no_focus_login));
				break;
//			case "8":
//				holder.tv_wait_money.setText("已结束");
//				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.title_no_focus_login));
//				break;
			case "9":
				...
				break;
			default:
				if (mMainToadayList.get(position).getFreezeStatus().equals("1")) {
					holder.tv_wait_money.setText("待确认付款");
	...
	private class Myadapter extends BaseAdapter {
		...
		public View getView(final int position, View convertview, ViewGroup arg2) {
            ...
            if (mMainEarlyList.get(position).getHeoTaskWorkCreateUsername()!=null&&mMainEarlyList.get(position).getHeoTaskWorkCreateUsername().length()>0) {
         	   holder.tv_name.setText("帮忙人:"+mMainEarlyList.get(position).getHeoTaskWorkCreateUsername());
			}else{
				 holder.tv_name.setText("帮忙人:"+"暂无领取人");
			}
            holder.tv_time.setText(PubMehods.getFormatDate(Long.valueOf(mMainEarlyList.get(position).getCreateTime()), "MM/dd HH:mm"));
            switch (mMainEarlyList.get(position).getTaskStatus()) {
			case "1":
				holder.tv_wait_money.setText("待领取");
				holder.tv_wait_money.setTextColor(getResources().getColor(R.color.attence_later));
				break;
...
//				break;
			case "9":
				...
				break;
			default:
				if (mMainEarlyList.get(position).getTaskStatus().equals("8")) {
					holder.tv_wait_money.setText("已失效");
				}
				if (mMainEarlyList.get(position).getExpiredStatus().equals("1")) {
					holder.tv_wait_money.setText("已失效");
					
				}
				if (mMainEarlyList.get(position).getMisdeedStatus().equals("1")) {
					holder.tv_wait_money.setText("任务违规");
					
				}
				if (mMainEarlyList.get(position).getFreezeStatus().equals("1")) {
					holder.tv_wait_money.setText("待确认付款");
