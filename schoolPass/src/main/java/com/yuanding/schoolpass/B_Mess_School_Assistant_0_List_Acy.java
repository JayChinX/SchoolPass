...
import com.yuanding.schoolpass.service.Api.InterSchoolAssistantList;
...
/**
 * @author 作者 E-mail:
 * @version 创建时间：2016年10月24日 上午10:56:19
 * 委派考勤通知列表/通知列表/审核通知列表
 */
public class B_Mess_School_Assistant_0_List_Acy extends A_0_CpkBaseTitle_Navi {
    ...
    private List<Cpk_School_Assistant_List> mCourseList = null;
...
//  private DisplayImageOptions options;
    ...
    private String type,title_name,acy_list_type,img_course;//消息类型 1:考勤 2：审核 3：通知  4：课程表
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras() != null) {
            ...
            acy_list_type = getIntent().getExtras().getString("acy_list_type", "");//何种信息类型
            img_course = getIntent().getExtras().getString("img_course", "");
        }
        if (type != null && !type.equals("")) {
            String str = getResources().getString(R.string.str_school_assistant);
            setTitleText(str + " "+ title_name );
        }
...
//              R.drawable.ic_default_empty_bg);
        ...
        mCourseList = new ArrayList<Cpk_School_Assistant_List>();
    ...
    private void readCache() {
        ...
        jsonObject = maACache.getAsJSONObject(AppStrStatic.cache_key_schoolasstistant_list + type + A_0_App.USER_UNIQID);
        if (jsonObject != null && !A_0_App.getInstance().getNetWorkManager().isNetWorkConnected()) {// 说明有缓存
            showInfo(getList(jsonObject));
        } else {
            updateInfo();
        }
    }

    private List<Cpk_School_Assistant_List> getList(JSONObject jsonObject) {
    ...
    private void judge_read(List<Cpk_School_Assistant_List> list,boolean firsetEnter) {
        if(!acy_list_type.equals("2")){//审核不做全部未读处理
    ...
    private void showInfo(List<Cpk_School_Assistant_List> list) {
        // TODO Auto-generated method stub
        if (isFinishing())
            return;
        havaSuccessLoadData = true;   
        successData(list, false);
    }
    
    private void successData(List<Cpk_School_Assistant_List> mList,final boolean pullRefresh) {
...
//                setZuiYouText("全选");
            ...
            if (pullRefresh)
                PubMehods.showToastStr(B_Mess_School_Assistant_0_List_Acy.this,
                        "刷新成功");
        } else {
            showTitleBt(ZUI_RIGHT_TEXT, false);
            judge_read(mList,firsetEnterLoader);
    ...
    private void getNoticeListData(int page_no,final boolean pullRefresh,final boolean onlyUpdateCache) {
        A_0_App.getApi().getSchoolAssistantList(B_Mess_School_Assistant_0_List_Acy.this,type,
                A_0_App.USER_TOKEN,String.valueOf(page_no),new InterSchoolAssistantList() {
                    @Override
                    public void onSuccess(List<Cpk_School_Assistant_List> mList) {
                        if (isFinishing())
                            return;
                        if(!onlyUpdateCache){//更新缓存用
                            havaSuccessLoadData = true;   
                            successData(mList, pullRefresh);
                        }
                    }
                },new Inter_Call_Back() {
                    ...
                    public void onFailure(String msg) {
                        ...
                        //new add
                        PubMehods.showToastStr(B_Mess_School_Assistant_0_List_Acy.this, msg);
                        demo_swiperefreshlayout.setRefreshing(false);  
                        if(null!=mPullDownView){
                            mPullDownView.onRefreshComplete();
                            mPullDownView.setMode(Mode.PULL_UP_TO_REFRESH);
    ...
    //全部设置为已读、批量更新
    private void batchUpdate(String message_type) {
        if (isFinishing())
            return;
        A_0_App.getInstance().showProgreDialog(B_Mess_School_Assistant_0_List_Acy.this, "", true);
        A_0_App.getApi().updateSchool_AssistantInfo(A_0_App.USER_TOKEN,message_type,
                new Inter_UpLoad_ChannelId() {
                    @Override
                    public void onSuccess(String message) {
                        if (isFinishing())
                            return;
                        A_0_App.getInstance().CancelProgreDialog(
                                B_Mess_School_Assistant_0_List_Acy.this);
                        for (int i = 0; i < mCourseList.size(); i++) {
                            mCourseList.get(i).setIs_read("1");// 全部设置为已读
                            adapter.notifyDataSetChanged();
                        }
                        no_Read_Count=0;
                        showTitleBt(ZUI_RIGHT_TEXT, false);
                        PubMehods.showToastStr(B_Mess_School_Assistant_0_List_Acy.this, message);
                    }
                },new Inter_Call_Back() {
        ...
        A_0_App.getApi().NoticeOfficalDelete(A_0_App.USER_TOKEN, messgesid,
                new InterNoticeOfficalDelete() {
                    ...
                    public void onSuccess() {
                        ...
                        PubMehods.showToastStr(B_Mess_School_Assistant_0_List_Acy.this, "删除成功");
                    }
                },new Inter_Call_Back() {
    ...
    private void getMoreLecture(int page_no) {
        A_0_App.getApi().getSchoolAssistantList(B_Mess_School_Assistant_0_List_Acy.this,type,
                ...
                new InterSchoolAssistantList() {
                    @Override
                    public void onSuccess(List<Cpk_School_Assistant_List> mList) {
                        if (isFinishing())
                            return;
                        
                        //A_0_App.getInstance().CancelProgreDialog(B_Mess_School_Assistant_0_List_Acy.this);
                        if (mList != null && mList.size() > 0) {
                            have_read_page += 1;
    ...
    public class Mydapter extends BaseAdapter {
        ...
        public View getView(final int posi, View converView, ViewGroup arg2) {
            ...
            if (converView == null) {
                converView = LayoutInflater.from(B_Mess_School_Assistant_0_List_Acy.this)
            ...
            if (LONGDELETE == 1) {
                RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(
                        ...
                        ViewGroup.LayoutParams.WRAP_CONTENT);
                int left=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 20);
                int top=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 20);
                int right=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 40);
                int bottom=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 1);
                layoutParams.setMargins(-left, top, right, bottom);// 4个参数按顺序分别是左上右下
                holder.ll_notice.setLayoutParams(layoutParams);
                int paddingLeft=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 40);
                int padding=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 17);
                ...
                int left=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 20);
                int top=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 20);
                int right=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 0);
                int bottom=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 1);
                layoutParams.setMargins(left, top, right, bottom);// 4个参数按顺序分别是左上右下
                int padding=DensityUtils.dip2px(B_Mess_School_Assistant_0_List_Acy.this, 17);
...
//            }
            ...
            if (mCourseList.get(posi).getIs_read().equals("1")) {
                holder.iv_notice_read_tag.setVisibility(View.GONE);
            } else {
                holder.iv_notice_read_tag.setVisibility(View.VISIBLE);
                holder.iv_notice_read_tag.setBackgroundResource(R.drawable.icon_message_noread);
            }
            holder.tv_notice_detail_title.setText(mCourseList.get(posi).getTitle());
            
            
            //设置item的内容
//            holder.tv_notice_deatil_content.setText(mCourseList.get(posi).getContent());
            String content = mCourseList.get(posi).getContent();
            if (null != content && content.length() > 0) {
                SpannableStringBuilder builder = PubMehods.splitStrWhereStr(B_Mess_School_Assistant_0_List_Acy.this, R.color.main_color, content, "\\{#", "#\\}");
                if(builder != null){
                    holder.tv_notice_deatil_content.setText(builder); 
                }
            }  
...
//            holder.tv_notice_deatil_from.setText(mCourseList.get(posi).getApp_msg_sign());
...
//            });
            ...
            holder.ll_notice.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View arg0) {
                    click_posi = posi;
                    mCourseList.get(click_posi).setIs_read("1");
                    //消息类型 1:考勤 2：审核 3：通知
                    if(mCourseList.get(posi).getType().equals("1")){
                        Intent intent = new Intent(B_Mess_School_Assistant_0_List_Acy.this,B_Mess_School_Assistant_1_Detai_Acy.class);
                        intent.putExtra("acy_type", 2);
                        intent.putExtra("message_id", mCourseList.get(posi).getMessage_id());
                        startActivityForResult(intent, 1);
                    }else if(mCourseList.get(posi).getType().equals("2")){
//                        Intent intent = new Intent(B_Mess_School_Assistant_0_List_Acy.this,B_Mess_School_Assistant_3_Detai_Examine_Acy.class);
//                        intent.putExtra("acy_type", 2);
//                        intent.putExtra("message_id", mCourseList.get(posi).getMessage_id());
//                        startActivityForResult(intent, 1);
                    }else if(mCourseList.get(posi).getType().equals("3")){
                        if(mCourseList.get(posi).getJump_module().equals("SIDE")){
//                            Intent intent = new Intent(B_Mess_School_Assistant_0_List_Acy.this,B_Side_Notice_Main.class);
//                            intent.putExtra("acy_type", 2);// 正常列表进入
//                            intent.putExtra("notice_type",AppStrStatic.cache_key_notice_sent+A_0_App.USER_UNIQID);
//                            intent.putExtra("acy_detail_id", mCourseList.get(posi).getLink_id());
//                            intent.putExtra("isHaveSend", 1+""); //是否是已发送页面
//                            intent.putExtra("type", mCourseList.get(posi).getType());
//                            startActivity(intent);
                        }else if(mCourseList.get(posi).getJump_module().equals("INDEX")){
                            Intent intent = new Intent();
                            if (mCourseList.get(posi).getMessage_type().equals("1")) {
                                intent.setClass(B_Mess_School_Assistant_0_List_Acy.this, B_Mess_Notice_Detail.class);
                            } else if (mCourseList.get(posi).getMessage_type().equals("4")) {            
                                intent.setClass(B_Mess_School_Assistant_0_List_Acy.this, B_Mess_Notice_Detail_MessText.class);                   
                            }else{
                                intent.setClass(B_Mess_School_Assistant_0_List_Acy.this, B_Mess_Notice_Detail_Sys.class);
                            }
                            intent.putExtra("school_notice", mCourseList.get(posi).getMessage_id());
                            intent.putExtra("acy_type", 2);
                            intent.putExtra("message_id", mCourseList.get(posi).getLink_id());
                            startActivity(intent);
                        }
                    }else if(mCourseList.get(posi).getType().equals("4")){//课程表
                        Intent courseIntent = new Intent(B_Mess_School_Assistant_0_List_Acy.this,B_Side_Course_Acy.class);
                        courseIntent.putExtra("acy_type", 12);
                        courseIntent.putExtra("img_course", img_course);
                        courseIntent.putExtra("message_id", mCourseList.get(posi).getMessage_id());
                        startActivity(courseIntent);
                  }else if(mCourseList.get(posi).getType().equals("6")){//请销假
                        Intent intent = new Intent(B_Mess_School_Assistant_0_List_Acy.this,Pub_WebView_Banner_Acy.class);
                        intent.putExtra("title_text", mCourseList.get(posi).getTitle());// 正常列表进入
                        intent.putExtra("url_text", mCourseList.get(posi).getLeave_detail_url());
...
//              A_0_App.getInstance().showExitDialog(B_Mess_School_Assistant_0_List_Acy.this,
