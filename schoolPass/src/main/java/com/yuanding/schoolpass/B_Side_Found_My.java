...
/**
 * @author Jiaohaili 
 * @version 创建时间：2015年11月12日 下午3:18:18 我的————我的————我的
 */
public class B_Side_Found_My extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		viewone = inflater.inflate(R.layout.activity_side_lost_found_main, container,
				false);
		...
		IntentFilter intentFilter = new IntentFilter("found");
	...
	private void readCache() {
		...
		jsonObject = maACache
				.getAsJSONObject(AppStrStatic.cache_key_side_found_my+A_0_App.USER_UNIQID);
	...
	private void getLectureList(int page_no, final boolean pullRefresh) {
		A_0_App.getApi().getLostList(getActivity(),"0",A_0_App.USER_TOKEN, String.valueOf(page_no),
				new InterLostList() {
					@Override
					public void onSuccess(List<Cpk_Side_Lost_List> mList) {
						...
						if (mList != null && mList.size() > 0) {
							...
							showLoadResult(false, true, false, false);
							if (pullRefresh)
								PubMehods.showToastStr(getActivity(),
										"刷新成功");
						} else {
							showLoadResult(false, false, false, true);
						}
	...
	private void getMoreLecture(int page_no) {
		if (A_0_App.USER_TOKEN == null || A_0_App.USER_TOKEN.equals(""))
			return;
		A_0_App.getApi().getLostList(getActivity(),"0",
	...
	public class Mydapter extends BaseAdapter {
		...
		public View getView(final int posi, View converView, ViewGroup arg2) {
			...
			holder.tv_person_name.setText(mLecturesList.get(posi).getTrue_name());
			holder.tv_create_time.setText(PubMehods.getFormatDate(
					mLecturesList.get(posi).getCreate_time(), "MM/dd  HH:mm"));
			...
			holder.tv_lost_time.setText(PubMehods.getFormatDate(mLecturesList.get(posi).getLost_time(), "MM/dd  HH:mm"));
			...
			if (mLecturesList.get(posi).getPhoto_url().length()>0&&mLecturesList.get(posi).getPhoto_url()!="") {
				String photo_urls[]=mLecturesList.get(posi).getPhoto_url().split(",");
				...
				if (photo_urls.length==1) {
		            ...
		            image1.setOnClickListener(new OnClickListener() {
		                ...
		                public void onClick(View arg0) {
		                	  Intent intent = new Intent(getActivity(), B_Side_Found_BigImage.class);
							...
							Bundle mBundle = new Bundle(); 
						     mBundle.putSerializable("content",(Serializable) mLecturesList.get(posi));
...
//			                        (int) (4 * density), (int) (4 * density));
			 ...
			 converView.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View arg0) {
							...
							Intent intent = new Intent(getActivity(),B_Side_Found_Detail.class);
							Bundle mBundle = new Bundle(); 
						     mBundle.putSerializable("content",(Serializable) mLecturesList.get(posi));
				             intent.putExtras(mBundle);
							startActivity(intent);
	...
	public  void callSb(final String lost_id,final int posi ) {
		final GeneralDialog upDateDialog = new GeneralDialog(getActivity(),R.style.Theme_GeneralDialog);
		...
		A_0_App.getApi().SideFoundDelete(A_0_App.USER_TOKEN, lost_id,
				new InterSideFoundDelete() {
					...
					public void onSuccess() {
                        if (mLecturesList == null)
                            return;
						Intent intent1 = new Intent("found_found");
